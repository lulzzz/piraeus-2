...
namespace SkunkLab.Channels
{
    public abstract class ChannelFactory
    {
        ...
        public static IChannel Create(bool usePrefixLength, TcpClient client, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, client, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, TcpClient client, X509Certificate2 certificate, bool clientAuth, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, client, certificate, clientAuth, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, TcpClient client, TlsPskIdentityManager pskManager, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
        ...
        public static IChannel Create(bool usePrefixLength, string hostname, int port, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, hostname, port, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, string hostname, int port, IPEndPoint localEP, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, hostname, port, localEP, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPEndPoint remoteEndpoint, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, remoteEndpoint, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPEndPoint remoteEndpoint, IPEndPoint localEP, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, remoteEndpoint, localEP, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPAddress address, int port, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, address, port, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPAddress address, int port, IPEndPoint localEP, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, address, port, localEP, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, string hostname, int port, X509Certificate2 certificate, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, hostname, port, certificate, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, string hostname, int port, IPEndPoint localEP, X509Certificate2 certificate, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, hostname, port, localEP, certificate, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPEndPoint remoteEndpoint, X509Certificate2 certificate, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, remoteEndpoint, certificate, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPEndPoint remoteEndpoint, IPEndPoint localEP, X509Certificate2 certificate, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, remoteEndpoint, localEP, certificate, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPAddress address, int port, X509Certificate2 certificate, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, address, port, certificate, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPAddress address, int port, IPEndPoint localEP, X509Certificate2 certificate, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, address, port, localEP, certificate, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPAddress address, int port, IPEndPoint localEP, string pskIdentity, byte[] psk, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, address, port, localEP, pskIdentity, psk, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPAddress address, int port, string pskIdentity, byte[] psk, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, address, port, pskIdentity, psk, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, string hostname, int port, IPEndPoint localEP, string pskIdentity, byte[] psk, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, hostname, port, localEP, pskIdentity, psk, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, string hostname, int port, string pskIdentity, byte[] psk, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, hostname, port, pskIdentity, psk, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPEndPoint remoteEP, IPEndPoint localEP, string pskIdentity, byte[] psk, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
            return TcpChannel.Create(usePrefixLength, remoteEP, localEP, pskIdentity, psk, blockSize, maxBufferSize, token);
        }

        public static IChannel Create(bool usePrefixLength, IPEndPoint remoteEP, string pskIdentity, byte[] psk, int blockSize = 0x4000, int maxBufferSize = 0x400000, CancellationToken token = default)
        {
        ...
        public static IChannel Create(string endpoint, string securityToken, IEnumerable<Observer> observers, CancellationToken token = default)
        {
            return HttpChannel.Create(endpoint, securityToken, observers, token);
        }

        public static IChannel Create(string endpoint, X509Certificate2 certificate, IEnumerable<Observer> observers, CancellationToken token = default)
        {
